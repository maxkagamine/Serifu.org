...
public partial class ElasticsearchService : IElasticsearchService
{
    ...
    public async Task<SearchResults> Search(string query, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                // Elasticsearch's .NET library is rather annoying and unconventional: BoolQuery and MatchQuery etc. are
                // NOT subclasses of Query as you'd expect. Assignment to Query invokes an implicit conversion, and for
                // some reason the only way to access the actual query object is like this.
